...
int jog_facil(int *linha, int *coluna, int tab[3][3]){
    srand(time(NULL));
    int valid=0, l, c;
    do{
        l = rand() % 3;
...
int jog_medio(int *linha, int *coluna, int tab[3][3], int fig){
    int i, j, vef=0;
    // Verifica linhas para ganhar
    for (i = 0; i < 3; i++) {
        if (tab[i][0] == tab[i][1] || tab[i][1] == tab[i][2] || tab[i][0] == tab[i][2]){
            if(tab[i][0]==fig||tab[i][1]==fig||tab[i][2]==fig){
                for(j=0;j<3;j++){
                    if(tab[i][j]==0){
                        *linha = i;
                        *coluna = j;
    ...
    for (j = 0; j < 3; j++) {
        if (tab[0][j] == tab[1][j] || tab[1][j] == tab[2][j] || tab[0][j] == tab[2][j] && vef==0){
            if(tab[0][j]==fig||tab[1][j]==fig||tab[2][j]==fig){
                for(i=0;i<3;i++){
                    if(tab[i][j]==0){
                        *linha = i;
                        *coluna = j;
